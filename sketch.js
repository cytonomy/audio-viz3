...
const frequencyRanges = [
  ...
  { name: "Sub Bass", min: 20, max: 40, color: [128, 0, 0], threshold: 0.1, group: "bass" },      // Dark Red
  { name: "Deep Bass", min: 40, max: 80, color: [255, 0, 0], threshold: 0.1, group: "bass" },     // Bright Red
  { name: "Bass", min: 80, max: 160, color: [255, 64, 0], threshold: 0.2, group: "bass" },        // Red-Orange
  { name: "Upper Bass", min: 160, max: 300, color: [255, 128, 0], threshold: 0.2, group: "bass" }, // Orange
  ...
  { name: "Mid", min: 800, max: 1200, color: [180, 255, 0], threshold: 0.25, group: "mid" },       // Yellow-Green
  { name: "Mid-High", min: 1200, max: 2000, color: [0, 255, 0], threshold: 0.22, group: "mid" },   // Green
  { name: "High Mids", min: 2000, max: 3000, color: [0, 255, 128], threshold: 0.18, group: "mid" }, // Blue-Green

  // HIGH GROUP - EXTREMELY LOW thresholds (ultra sensitive)
  { name: "Low Treble", min: 3000, max: 4000, color: [0, 255, 255], threshold: 0.08, group: "high" }, // Cyan
  { name: "Mid Treble", min: 4000, max: 6000, color: [0, 128, 255], threshold: 0.06, group: "high" }, // Light Blue
  { name: "Presence", min: 6000, max: 8000, color: [0, 64, 255], threshold: 0.04, group: "high" },  // Blue
  { name: "Brilliance", min: 8000, max: 12000, color: [128, 0, 255], threshold: 0.03, group: "high" }, // Purple
  { name: "Air", min: 12000, max: 20000, color: [255, 0, 255], threshold: 0.02, group: "high" }     // Magenta
];
...
function analyzeAudio() {
  if (audioInitialized && analyser && dataArray) {
    ...
    audioLevel = pow(audioLevel, 0.7); // Apply curve for better response
  } else if (fallbackMode) {
    // Simulate audio in fallback mode
    audioLevel = 0.3 + 0.3 * sin(frameCount * 0.05);
    // Add some random variation
    if (random(1) < 0.05) {
      audioLevel = random(0.2, 0.8);
    }
  }
}
...
function spawnParticlesBasedOnFrequencies() {
  ...
  for (let i = 0; i < frequencyRanges.length; i++) {
    ...
    const relativeEnergy = 4*energy / range.threshold;
  ...
  if (dominantRange) {
    ...
    let count = floor(map(energy, dominantRange.threshold, 1, 1, 20));
...
function createColoredParticle(range, energy) {
  ...
  const xVariation = width * 0.1; // Add some variation
  ...
  if (range.group === "bass") {
    ...
    p.maxSpeed = map(energy, range.threshold, 1, 1.0, 3.5);
  } else if (range.group === "high") {
    // High frequencies: get fast with less energy
    p.maxSpeed = map(energy, range.threshold, 0.5, 2.0, 5.0);
  } else {
    // Mid frequencies: normal scaling
    p.maxSpeed = map(energy, range.threshold, 1, 1.5, 4.0);
  }
  
  // Constrain speed to reasonable values
  p.maxSpeed = constrain(p.maxSpeed, 1.0, 5.0);
